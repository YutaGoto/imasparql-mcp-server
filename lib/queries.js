...
export async function getEntity(params) {
  ...
  const sparql = `
    ...
    SELECT ?label ?type ?nameKana ?name ?height ?weight ?gender ?age ?blood ?brand ?bust ?waist ?hip ?handedness ?hobby ?birthPlace ?birthDate ?constellation ?grade ?color ?cv
    WHERE {
      VALUES ?s { <${uri}> }
...
export async function getUnitMembers(params) {
  const uri = params?.id;
  if (!uri) return res.json({ jsonrpc: "2.0", id, error: "Missing id" });
...
export async function getUnitMembersByName(params) {
  ...
  if (!name) return res.json({ jsonrpc: "2.0", id, error: "Missing name" });
  ...
  const sparql = `
    ...
    WHERE {
      ...
      FILTER (
        CONTAINS(LCASE(STR(?unitLabel)), LCASE("${name}"))
      )
