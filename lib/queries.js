...
export async function searchEntities(params) {
  ...
  if (birthDateFrom && birthDateTo) {
    const op = birthDateFrom <= birthDateTo ? "&&" : "||";
    birthDateFilter = `FILTER(STR(?birthDate) >= "--${birthDateFrom}" ${op} STR(?birthDate) <= "--${birthDateTo}")`;
  } else if (birthDateFrom) {
    birthDateFilter = `FILTER(STR(?birthDate) >= "--${birthDateFrom}")`;
  } else if (birthDateTo) {
    birthDateFilter = `FILTER(STR(?birthDate) <= "--${birthDateTo}")`;
  }
