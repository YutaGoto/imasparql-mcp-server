...
export async function searchEntities(params) {
  ...
  const birthMonth = params?.birthMonth;
  const birthDay = params?.birthDay;
  const birthDateFrom = params?.birthDateFrom;
  const birthDateTo = params?.birthDateTo;
  ...
  const sparql = `
    ...
    WHERE {
      ...
      ${birthMonth ? `FILTER(STRSTARTS(STR(?birthDate), "--${String(birthMonth).padStart(2, "0")}-"))` : ""}
      ${birthDay ? `FILTER(STRENDS(STR(?birthDate), "-${String(birthDay).padStart(2, "0")}"))` : ""}
      ${birthDateFrom && birthDateTo ? (birthDateFrom <= birthDateTo ? `FILTER(STR(?birthDate) >= "--${birthDateFrom}" && STR(?birthDate) <= "--${birthDateTo}")` : `FILTER(STR(?birthDate) >= "--${birthDateFrom}" || STR(?birthDate) <= "--${birthDateTo}")`) : ""}
      ${birthDateFrom && !birthDateTo ? `FILTER(STR(?birthDate) >= "--${birthDateFrom}")` : ""}
      ${!birthDateFrom && birthDateTo ? `FILTER(STR(?birthDate) <= "--${birthDateTo}")` : ""}
    }
